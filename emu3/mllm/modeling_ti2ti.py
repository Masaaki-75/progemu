# coding=utf-8
# Adapted from https://github.com/baaivision/Emu3/blob/main/emu3/mllm/modeling_emu3.py
...
def _get_unpad_data(attention_mask):
    """
    Returns:
        indices: A flattened list of indices in the attention_mask that correspond to real (non-padded) tokens.
        cu_seqlens: A prefix sum (cumulative sum) of sequence lengths in the batch, padded with a 0 at the beginning. 
            This indicates where each sequence starts and ends in the *flattened* indices.
        max_seqlen_in_batch: The maximum sequence length in the batch (excluding padding).
    """
    seqlens_in_batch = attention_mask.sum(dim=-1, dtype=torch.int32)  # valid seq lengths (excluding padding tokens)
    indices = torch.nonzero(attention_mask.flatten(), as_tuple=False).flatten()
    max_seqlen_in_batch = seqlens_in_batch.max().item()
    cu_seqlens = F.pad(torch.cumsum(seqlens_in_batch, dim=0, dtype=torch.torch.int32), (1, 0))  # add a starting zero
    return (
        indices,
        cu_seqlens,
        max_seqlen_in_batch,
...
class Emu3FlashAttention2(Emu3Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        output_attentions = False  # [NOTE] hardcode this because flashattn2 does not support computing attention maps
        ...
        # [batch_size, sequence_length, num_heads, head_dim]. We would need to refactor the KV cache to 
        # be able to avoid many of these transpose/reshape/view.
    ...
    def _flash_attention_forward(
        ...
        if attention_mask is not None:
            batch_size = query_states.shape[0]  # [batch_size, max_len, 32, 128]
            query_states, key_states, value_states, indices_q, cu_seq_lens, max_seq_lens = self._upad_input(
                query_states, key_states, value_states, attention_mask, query_length
            )
            # q,k,v are unpadded tensors (extracting the non-padding tokens)
...
class Emu3Model(Emu3PreTrainedModel):
    ...
    def forward(
        ...
        are_image_ids: torch.LongTensor = None,  # specify which tokens are image tokens (to create non-causal mask on them)
        ...
        if self._use_flash_attention_2:
            ...
            # During inference, due to torch.all(attention_mask == 1) all the time,
            # the `ignore_causal_mask` will be set to True, resulting in `attention_mask==None`.
            # So, we need to avoid this exception when dealing with are_image_ids.
            # (Or should we? Let's try always with `attention_mask==None`)
            attention_mask_keep = attention_mask.clone()
            ...
            if attention_mask is None and past_key_values_length != 0:
                ...
                )  # [1, 1, q_len=1, q_len+past_key_value_length]

            if are_image_ids is not None and attention_mask is not None:
                image_mask = (are_image_ids.unsqueeze(2) * are_image_ids.unsqueeze(1)).unsqueeze(1)  # [batch_size, 1, seq_len, seq_len]
                image_mask = image_mask.to(attention_mask.dtype)
                attention_mask = (1 - image_mask) * attention_mask
        else:
            # 4d mask is passed through the layers
            attention_mask = _prepare_4d_causal_attention_mask(
                attention_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length
            )
            if are_image_ids is not None:
                image_mask = (are_image_ids.unsqueeze(2) * are_image_ids.unsqueeze(1)).unsqueeze(1)  # [batch_size, 1, seq_len, seq_len]
                image_mask = image_mask.to(attention_mask.dtype)
                attention_mask = (1 - image_mask) * attention_mask
...
class Emu3ForCausalLM(Emu3PreTrainedModel):
    ...
    def forward(
        self,
        input_ids: torch.LongTensor = None,  # [batch_size, max_len]
        attention_mask: Optional[torch.Tensor] = None,  # 0 for padded, 1 for unpadded, [batch_size, max_len]
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        are_image_ids: torch.LongTensor = None,
        ...
        model_inputs.update(
            {
                ...
                "attention_mask": attention_mask,
                "are_image_ids": kwargs.get("are_image_ids", None)
            }
        )
        return model_inputs
